...
impl<'ui> Gui<'ui> {
    ...
    fn draw_me3_plot_category(&self, plot_table: &mut PlotTable, plot_db: &PlotCategory) {
        let ui = self.ui;
        let PlotCategory { booleans, integers } = plot_db;

        if booleans.is_empty() && integers.is_empty() {
            return;
        }
        ...
        // Integers
        let mut clipper = ListClipper::new(integers.len() as i32).begin(ui);
        while clipper.step() {
            for i in clipper.display_start()..clipper.display_end() {
                let (plot_id, plot_desc) = integers.get_index(i as usize).unwrap();
